...
const props = withDefaults(
  defineProps<{ icon: string; label: string; tone?: Tone; size?: Size; active?: boolean }>(),
...
<template>
  <button
    ...
    :title="label"
    :class="[
      'inline-grid place-items-center rounded-control transition duration-150 active:scale-90',
      'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--accent-ring)]',
      sizes[size],
      active ? 'bg-surface-2 text-ink' : tones[tone],
    ]"
  >
    <UiIcon :name="icon" :size="size === 'sm' ? 15 : 16" />
  </button>
</template>
