      ...
      <div class="rounded-2xl border border-line bg-surface p-7 shadow-card">
        ...
        <form class="mt-6 flex flex-col gap-4" @submit.prevent="submit">
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <input v-model="password" type="password" required autocomplete="current-password" placeholder="••••••••" class="field-input">
          </div>
          ...
          <UiButton variant="primary" type="submit" size="lg" block :loading="busy">Log in</UiButton>
        </form>
      </div>

      <p class="mt-5 text-center text-sm text-ink-muted">
        No account?
        <NuxtLink to="/register" class="font-medium text-accent hover:underline">Create one</NuxtLink>
      </p>
    </div>
  </div>
</template>
