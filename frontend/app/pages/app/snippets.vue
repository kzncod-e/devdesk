...
const filters = computed(() => {
  ...
  if (collectionFilter.value !== null)
    params.set("collection_id", String(collectionFilter.value));
...
const { intent: quickCreateIntent, consume: consumeQuickCreate } =
  useQuickCreate();
watch(
  quickCreateIntent,
  () => {
...
<template>
  <div class="mx-auto max-w-6xl px-5 py-8 md:px-8">
    <header class="mb-7 flex flex-wrap items-end justify-between gap-4">
      <div>
        <h1 class="text-title">Snippets</h1>
    ...
    <div class="flex flex-col gap-6 lg:flex-row lg:items-start">
      ...
      <aside class="w-full lg:w-56 lg:shrink-0">
        <div class="rounded-card border border-line bg-surface p-3">
          <p
            class="px-1 pb-2 text-[11px] font-semibold uppercase tracking-wider text-ink-subtle"
          >
            Folders
          </p>
          <button
          ...
          >
            ...
            >
              <UiIcon
                name="trash"
                :size="13"
                class="text-ink-subtle hover:text-red-500"
              />
            </button>
          </div>

          <form
            class="mt-2 flex gap-1.5 border-t border-line pt-2"
      ...
      <div class="min-w-0 flex-1">
        <div class="mb-5 flex flex-wrap items-center gap-3">
          ...
          <UiMenu align="right">
            <template #trigger>
              <UiButton variant="ghost" size="sm" icon="bookmark"
                >Views</UiButton
              >
            </template>
            <p
            ...
            <p
              ...
              class="px-2.5 py-1.5 text-xs text-ink-subtle"
            >
              No saved views yet.
            </p>
            <div
            ...
            >
              ...
              <button
                type="button"
                class="px-1.5 text-ink-subtle hover:text-red-500"
    ...
    <UiModal :open="showForm" no-header width="max-w-2xl" @close="closeForm">
      <SnippetForm
        v-if="showForm"
        :key="editing?.id ?? 'new'"
        :snippet="editing"
    ...
    >
      <form class="flex flex-col" @submit.prevent="saveCurrentView">
        ...
        >
          <UiButton variant="ghost" type="button" @click="showSaveView = false"
            >Cancel</UiButton
          >
          <UiButton
            variant="primary"
