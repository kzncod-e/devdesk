...
class BookmarkService:
    ...
    async def create(self, *, owner_id: int, url: str, tags: list[str],
                     project_id: int | None) -> dict:
        await self._validate_project(project_id, owner_id)
        return await self.repo.create(owner_id=owner_id, url=url, tags=tags,
    ...
    async def get(self, bookmark_id: str, owner_id: int) -> dict:
        doc = await self.repo.get(bookmark_id, owner_id)
        if doc is None:
            raise NotFoundError("Bookmark not found")
        return doc

    async def update(self, bookmark_id: str, owner_id: int, *, fields: dict) -> dict:
        if "project_id" in fields:
            await self._validate_project(fields["project_id"], owner_id)
        doc = await self.repo.update(bookmark_id, owner_id, fields=fields)
        if doc is None:
            raise NotFoundError("Bookmark not found")
        return doc

    async def delete(self, bookmark_id: str, owner_id: int) -> None:
        if not await self.repo.delete(bookmark_id, owner_id):
            raise NotFoundError("Bookmark not found")
