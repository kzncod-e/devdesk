...
class SnippetService:
    ...
    async def get(self, snippet_id: str, workspace_id: int) -> dict:
        doc = await self.repo.get(snippet_id, workspace_id)
        if doc is None:
            raise NotFoundError("Snippet not found")
        return doc

    async def update(self, snippet_id: str, workspace_id: int, *, fields: dict) -> dict:
    ...
    async def delete(self, snippet_id: str, workspace_id: int) -> None:
    ...
    async def _validate_project(self, project_id: int | None, workspace_id: int) -> None:
        # cross-database reference check: Mongo docs may only point at projects
        # that live in the same workspace
